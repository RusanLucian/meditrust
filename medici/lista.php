...
// Fetch doctors with specialties, average rating and review count
$query = "
    SELECT 
        u.id, 
        u.name, 
...
if (!empty($search_name)) {
    $filtered_doctors = array_filter($filtered_doctors, function ($d) use ($search_name) {
...
<option value="3" <?php echo (string)$selected_rating === '3' ? 'selected' : ''; ?>>
⭐ 3+
</option>

<option value="4" <?php echo (string)$selected_rating === '4' ? 'selected' : ''; ?>>
⭐ 4+
</option>

<option value="5" <?php echo (string)$selected_rating === '5' ? 'selected' : ''; ?>>
⭐ 5
</option>
...
<?php if (!empty($selected_specialty) || !empty($selected_rating) || !empty($search_name)): ?>

<a href="lista.php" class="back-btn">
Resetează filtrele
</a>
...
Nu am găsit medici pentru filtrele selectate.
...
<?php echo htmlspecialchars($doctor['specialty'] ?? 'Specialitate indisponibilă'); ?>

</p>

<p>

⭐ <?php echo number_format((float)$doctor['avg_rating'], 1); ?>/5

(<?php echo (int)$doctor['review_count']; ?> recenzii)
...
<a
href="detalii.php?id=<?php echo (int)$doctor['id']; ?>"
class="btn-view"
>
